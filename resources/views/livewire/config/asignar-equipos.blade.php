    ...
    <x-slot name="header">
     ...
     <div class="mb-4 bg-gray-900 p-4 rounded-lg">
         ...
         <select wire:model.live="grupoSeleccionado"
             class="block w-full border-gray-100 bg-gray-100 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500">
     ...
     {{-- Selección de equipos si hay grupo seleccionado o es todos contra todos --}}
     @if ($campeonato->formato === 'todos_contra_todos' || $grupoSeleccionado)
     <div class="mb-4 bg-gray-900  p-4 rounded-lg">
         <label class="block mb-2 text-sm font-medium text-gray-100 dark:text-white">
             Seleccionar equipos {{ $campeonato->formato === 'grupos' ? 'para el grupo' : 'para el campeonato' }}
...
@endadminOrCan
             ...
             <div class="p-4 border rounded-lg bg-gray-100 shadow-yellow-100 ">
                 ...
                 <ul class="list-disc list-inside space-y-1">
                     ...
                     <li>
                         ...
                         <button type="button"
                             wire:click="removerEquipoDeGrupo({{ $equipo->id }}, {{ $grupo->id }})"
                             class="text-red-600 hover:underline text-sm ml-2 cursor-pointer">
                             <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-5 w-5 shasow-lg">
     ...
     {{-- Mostrar equipos asignados directamente al campeonato (todos contra todos) --}}
     @if ($campeonato->formato === 'todos_contra_todos')
     <div class="mt-6">
         <h2 class="block mb-2 text-lg font-medium text-gray-900 dark:text-white">Equipos del Campeonato (Todos
             contra todos)</h2>
     ...
     <script>
         document.addEventListener('livewire:initialized', () => {
             ...
             Livewire.on('equiposAsignados', () => {
                 Swal.fire(
                     '¡Éxito!',
                     'El equipo a sido asignado correctamente.',
                     'success'
                 );
             });
             ...
             Livewire.on('errorEquipoAsignado', (grupoNombre) => {
                 Swal.fire({
                     ...
                     // Asegúrate de que las variables se usen en el lugar correcto
