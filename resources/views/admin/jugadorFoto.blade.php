<x-app-layout>
    <div class="max-w-md mx-auto mt-10">
        <div class="flex items-center p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400" role="alert">
            ...
            <div>
                <h1> <span class="font-medium underline decoration-blue-500">Cargar foto para</span>: {{ strtoupper($jugador->apellido) }} {{strtoupper($jugador->nombre)}}</h1>
            </div>
        </div>
        @if(session('success'))
        ...
        <div class="mb-4 flex justify-center">
            <div class="w-32 h-32 rounded-full border-4 border-gray-300 shadow-lg overflow-hidden">
                <img src="{{ asset('storage/' . $jugador->foto) }}"
                    alt="Foto {{ $jugador->nombre }}"
                    class="w-full h-full object-cover" />
            </div>
        </div>
        @endif

        <form action="{{ route('jugadores.foto.guardar', $jugador->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="flex flex-col items-center justify-center w-full">
                <label for="file-input" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                    ...
                    <input type="file" id="file-input" name="foto" onchange="previewImage(event)" class="hidden" />
            ...
            <div class="flex justify-between mt-4">
                ...
                <a href="{{ route('jugadores.index') }}" class="bg-blue-950 text-white px-4 py-2 rounded flex items-center gap-2 hover:bg-blue-800"> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
